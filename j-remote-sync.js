(()=>{
  ...
  function safeVideo(u){try{const x=new URL(String(u));return x.hostname==='www.youtube.com'&&x.pathname.startsWith('/embed/')?x.href:''}catch{return ''}}
  function show(d){
    ...
    const v=safeVideo(d.video_url);if(v)html+=`<iframe class="jRemoteVideo" src="${esc(v)}" title="J video" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>`;
    body.innerHTML=html;
